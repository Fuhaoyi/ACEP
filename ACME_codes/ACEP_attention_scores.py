...
att_p10_T = attention_weights_p10
att_p10_T_pd = pd.DataFrame(data=att_p10_T,index=seq_lab,
                            columns=pos_lab)
...
att_wei = attention_weights[:,20:40].flatten()
...
g = sns.barplot(x="pos", y="att_wei", data=att_pd, ax=axes)
axes.set(ylim=(0, 0.3), ylabel="", xlabel="")
plt.title('Average attention intensity of 712 AMPs',fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.setp(g.get_yticklabels(), rotation='horizontal')
plt.setp(g.get_xticklabels(), rotation='vertical')
plt.savefig('experiment_results\\Average_AMP712_1.png',dpi=600,format='png')
plt.show()
...
att_pos = att_pos*712
att_pos_cata = pd.Categorical(att_pos)
att_wei = attention_weights.flatten()
att_pd = pd.DataFrame({'pos':att_pos_cata,'att_wei':att_wei})
print(att_pd)
...
axes.set(xlim=(0, 0.3), ylabel="",
        xlabel="")
plt.title('Average attention intensity of 712 AMPs',fontsize=14)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.setp(g.get_yticklabels(), rotation='horizontal')
plt.setp(g.get_xticklabels(), rotation='vertical')
plt.savefig('experiment_results\\Average_AMP712_2.png',dpi=600,format='png')
plt.show()
